...
def _apply_default_arguments(args):
    ...
    # By default, pedantic diagnostics are enabled, but only when assertions in
    ...
    # checks.
    if args.pedantic_diagnostics is None:
        args.pedantic_diagnostics = args.swift_assertions
...
def create_argument_parser():
    ...
    option('--pedantic-diagnostics', store,
           const=True,
           help='Enable and escalate certain compiler warnings for code health '
                ...
                'enabled)')
    option('--no-pedantic-diagnostics', store('pedantic_diagnostics'),
           const=False)
